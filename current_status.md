...
J2 Comfort Sleep has been removed from the component. On the tested J2 it is available from the physical IR remote, but the genuine Toshiba app exposes only ordinary ON/OFF timers and no Comfort Sleep control. A timed Focused Monitor capture around an IR Comfort Sleep command showed no corresponding UART publication of `F7`, `F8`, `0x87`, `0x94` or `0x96`; only a later Auto-fan state publication was visible as a consequence. The project therefore treats Comfort Sleep as an IDU-internal/IR function rather than an exposed UART control unless future direct evidence establishes otherwise.
