...
J2 Comfort Sleep is exposed as a duration selector (`Off`, `1 hour`, `3 hours`, `5 hours`, `9 hours`). On the tested J2 unit the genuine Toshiba app exposes ordinary ON/OFF timers but no Comfort Sleep control, so Comfort Sleep is currently treated as a remote-control-only function for protocol investigation. The current implementation deliberately reproduces the directly observed RC result rather than assigning an unverified F7 value: Power Select 50%, fan Auto, OFF-timer duration in `0x96`, then OFF-timer enable in `0x94`. This recipe remains hardware-testable/falsifiable; the 1-hour RC capture showed the resulting 50%/Auto/OFF-timer-active state but did not itself publish `0x96`.

The universal package also exposes ordinary `ON Timer` and `OFF Timer` selects from the validated timer registers. Both selectors cover 30 minutes through 12 hours in 30-minute steps. Availability is gated from the climate operating state rather than cached `0x80`, because `0x80` is a logical/requested/armed state and can report ON while an ON timer is merely armed and the IDU is still physically stopped. The ON Timer accepts a non-Off duration only when the climate mode is OFF; the OFF Timer accepts a non-Off duration only when the climate mode is running. Cancelling with `Off` is allowed regardless of current state so an armed timer can always be cleared. A direct 30-minute ON-timer test with the ESP controller was ACKed for both `0x92` and `0x90`, read back `0x90 = 0x41`, and produced the expected armed `0x80 = 0x30` state while airflow remained zero. A later 10 h 30 min test (`0x92 = 0A 1E`), which is not an option exposed by the genuine remote, was likewise ACKed and armed by the IDU; expiry at that duration has not yet been observed. The OFF Timer path has also been reproduced directly from the ESP controller while the unit was running in Fan Only: `0x96 = 00 1E` (30 minutes) and `0x94 = 0x41` both received normal ACKs, and the IDU continued running with live fan feedback after arming; the eventual 30-minute shutdown was not part of that capture.
